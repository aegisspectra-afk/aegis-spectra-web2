...
function buildInvoiceHtml(orderId: string, order: OrderShape) {
...
export async function POST(req: NextRequest) {
  try {
    const { orderId, order } = (await req.json()) as { orderId: string; order: OrderShape };
    const safeOrderId = String(orderId || 'order');
    const html = buildInvoiceHtml(safeOrderId, order || {});

    try {
      // Try to render to PDF if Playwright is available in the environment
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { chromium } = (await import('playwright-core')) as unknown as {
        chromium: { launch: (opts: { args?: string[]; headless?: boolean }) => Promise<any> };
      };
